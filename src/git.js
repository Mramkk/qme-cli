...
const { getGitUser, getProjectRepoUrl, getCurrentBranch } = require("./utils.js");
...
async function runGitSync() {
    ...
    console.log(chalk.blueBright("🌿 Current branch:"), chalk.green(currentBranch));
    console.log(
...
async function runGitReset() {
    ...
    const resetFlag = action === "soft" ? "--soft" : action === "mixed" ? "--mixed" : "--hard";
    try {
...
async function runGitLogReset() {
    ...
    while (!selectedCommit) {
        ...
        for (let i = 0; i < commits.length; i += 1) {
            ...
            console.log(chalk.green(`  ${offset + i + 1}) ${item.hash} ${item.subject}`));
        }
        if (page.hasNext) {
        ...
        const selection = await askGitLogCommitSelection(pageStartIndex, pageEndIndex, {
    ...
    console.log();
    console.log(chalk.blue(`🔄 Selected: ${selectedCommit.hash} ${selectedCommit.subject}`));
    const resetMode = await askResetModeForCommit();

    try {
        execSync(`git reset ${resetMode} ${selectedCommit.hash}`, { stdio: "inherit" });
        console.log(chalk.green(`✅ Reset complete (${resetMode} ${selectedCommit.hash})`));
    } catch (error) {
        console.log(chalk.red(`❌ Reset failed: ${formatGitError(error)}`));
        process.exit(1);
    }
...
async function maybeShowStashMenu() {
    ...
    while (stashEntries.length > 0) {
        ...
        try {
            ...
            console.log(chalk.red(`❌ Stash action failed: ${formatGitError(error)}`));
        }
...
async function stashChanges(currentBranch) {
    try {
        ...
        execSync(`git stash push -u -m "${stashMessage.replace(/"/g, '\\"')}"`, { stdio: "inherit" });
...
function withTimestampPrefix(message) {
    ...
    const existingPrefixPattern = /^@?\[[A-Za-z]{3} \d{4}-\d{2}-\d{2} \d{2}:\d{2}\]\s*/;
...
async function doPull(remoteBranch, currentBranch) {
    ...
    try {
        execSync(`git pull ${REMOTE} ${remoteBranch} --no-edit`, { stdio: "inherit" });
...
function showLastCommits() {
    try {
        console.log();
        execSync("git --no-pager log -10 --decorate --oneline", { stdio: "inherit" });
    } catch {
